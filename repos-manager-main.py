...
class GitManager:
    ...
    @staticmethod
    def checkout_main_branch(base_folder, repo: Repo):
        FileManager.go_to_folder('{}/{}'.format(base_folder, repo.name))
        try:
            ConsoleOutput.action('Checking out {branch} for {repo_name}'.format(branch=MAIN_BRANCH, repo_name=repo.name))
            subprocess.run(['git', 'checkout', MAIN_BRANCH], stdout=subprocess.PIPE,
...
def process_repo(repo: Repo,
    ...
    # checkout main branch
        ...
        GitManager.checkout_main_branch(folder_path, repo)
